...
void GetInput(){
	...
	if(acnt>bcnt){
		...
		lop(i,acnt) scanf("%lf%lf",&pa[i].x,&pa[i].y);	//If N>M then swap the input
	}else{
...
int PseudoExpand(int sp1,int p1,int sp2,int p2,double excl){
	if(++extl>1e5) return 0;
	if(p1==acnt){
		if(excl+GetWeight(pa[sp1],pa[p1],pa[1],pb[sp2],pb[p2],pb[mt[1]])<lim) return 1;
		else return 0;
	}
	for(int j=p2+1;j<=bcnt+1-(acnt-p1);++j){
		double delta=GetWeight(pa[sp1],pa[p1],pa[p1+1],pb[sp2],pb[p2],pb[j]);
		if(excl+delta+3*(acnt-p1)<lim&&PseudoExpand(p1,p1+1,p2,j,excl+delta)) return 1;
	}
	return 0;
}																				//If have any vote,then return.
																				//Just For Binary Optimization.
int TryToExpand(int i,double x){
	lim=x;
	for(int j=preb;j<=bcnt;++j){
		if(i==1) mt[i]=j;
		if(PseudoExpand(i-1?i-1:i,i,mt[i-1]?mt[i-1]:j,j,0)) return 1;			//Similar to the main program
	}
	return 0;
}
signed main(signed argc,char** argv,char** env){
	...
	lop(i,acnt){
		...
		while(l+Eps<r){
			double mid=(l+r)/2;
			if(TryToExpand(i,mid)) r=mid;
			else l=mid;
		}															//Binary Search to get the least lim
		lim=r,extl=0;
		for(int j=preb;j<=bcnt;++j){
			if(i==1) mt[i]=j;
